...
unsigned int btn_map_logitech_dual_action[BTN_COUNT] = { 8, // BTN_QUIT
                                                         0, // BTN_INC_JOINT
                                                         1, // BTN_DEC_JOINT
...
std::string btn_labels_logitech_dual_action[12] = { "1",
                                                    "2",
                                                    "3",
                                                    "4",
                                                    "5",
...
unsigned int ax_map_logitech_dual_action[AX_COUNT] = { 1, // AX_POSITION
                                                       0, // AX_VELOCITY
                                                       3, // AX_TORQUE
                                                       2, // AX_MOTOR_CURRENT
                                                       1, // AX_INERTIA
...
std::string ax_labels_logitech_dual_action[6] = { "L Horz.",
                                                  "L Vert.",
                                                  "R Horz.",
                                                  "R Vert.",
                                                  "D Horz.",
...
std::string btn_labels_xbox_360_wireless[15] = { "A",
                                                 "B",
                                                 "X",
                                                 "Y",
                                                 "LB",
...
unsigned int btn_map_xbox_360_wireless[BTN_COUNT] = { 14,// BTN_QUIT
                                                      2, // BTN_INC_JOINT
                                                      0, // BTN_DEC_JOINT
                                                      3, // BTN_RESET_TARGET_POS_VEL
...
std::string ax_labels_xbox_360_wireless[6] = { "L Horz.",
                                               "L Vert.",
                                               "LT",
                                               "R Horz.",
                                               "R Vert.",
...
unsigned int ax_map_xbox_360_wireless[AX_COUNT] = { 1, // AX_POSITION
                                                    0, // AX_VELOCITY
                                                    4, // AX_TORQUE
                                                    3, // AX_MOTOR_CURRENT
                                                    1, // AX_INERTIA
...
std::string btn_labels_xbox_360_pad[11] = { "A",
                                            "B",
                                            "X",
                                            "Y",
                                            "LB",
...
unsigned int btn_map_xbox_360_pad[BTN_COUNT] = { 10,// BTN_QUIT
                                                 2, // BTN_INC_JOINT
                                                 0, // BTN_DEC_JOINT
                                                 3, // BTN_RESET_TARGET_POS_VEL
                                                 1, // BTN_RESET_TARGET_TOR_CUR
...
std::string ax_labels_xbox_360_pad[8] = { "L Horz.",
                                          "L Vert.",
                                          "LT",
                                          "R Horz.",
                                          "R Vert.",
...
unsigned int ax_map_xbox_360_pad[AX_COUNT] = { 1, // AX_POSITION
                                               0, // AX_VELOCITY
                                               4, // AX_TORQUE
                                               3, // AX_MOTOR_CURRENT
                                               1, // AX_INERTIA
...
{
   static const double ratio = 180.0 / M_PI;
   if (fabs(rad) > (M_PI)) return ratio * atan2(sin(rad),cos(rad));
   else return ratio * rad;
}
...
bool HDTManipulatorTestApp::SetInitialPosition(std::vector <double> initial_joint_angles)
{
    ...
    {
        printf("Error: initial_joint_angles size (%d) is different from num_joints (%d)\n",
               initial_joint_angles.size(), num_joints);
        return false;
    }
}

bool HDTManipulatorTestApp::SetInitialPositionDegrees(std::vector <double> initial_joint_angles_degrees)
{
...
bool HDTManipulatorTestApp::InitializeJoy()
{
   if(joy.Initialize(0)){
      ...
      else if (joy_name.find("Xbox") != std::string::npos || joy_name.find("X-Box") != std::string::npos)
      {
          btn_map = btn_map_xbox_360_wireless;
          btn_label = btn_labels_xbox_360_wireless;
          ax_map = ax_map_xbox_360_wireless;
   ...
   for (int ix=0; ix < BTN_COUNT; ix++) if (btn_label[btn_map[ix]].size() > max_btn_label) max_btn_label = btn_label[btn_map[ix]].size();
   for (int ix=0; ix < AX_COUNT; ix++) if (ax_label[ax_map[ix]].size() > max_ax_label) max_ax_label = ax_label[ax_map[ix]].size();
   return true;
}
...
{
   context_option.clear(); context_name.clear();
   ...
   context_option.push_back(POSITION_CONTROL);     context_name.push_back("Joint Position Control");
...
void HDTManipulatorTestApp::Update()
{
   ...
   if(joy.PressedButton(btn_map[BTN_QUIT]) || 'q' == ch || 'Q' == ch) run_flag = false;
   prev_context = curr_context;
   if('1' <= ch && ch <= '4' && ((int)(ch - '1')) < context_option.size()) curr_context = context_option[(int)(ch-'1')];

   //////////////////////////////////////////////////////////////////////////////
   // Use buttons labeled 1 & 2  and up/down arrow keys to select active joint //
   //////////////////////////////////////////////////////////////////////////////
   if(!joy.Button(btn_map[BTN_SEND_MOTION_CMD]) && (joy.PressedButton(btn_map[BTN_INC_JOINT]) || KEY_UP == ch)) curr_joint_ix = (curr_joint_ix+1)%num_joints;
   if(!joy.Button(btn_map[BTN_SEND_MOTION_CMD]) && (joy.PressedButton(btn_map[BTN_DEC_JOINT]) || KEY_DOWN == ch)) curr_joint_ix = (curr_joint_ix+(num_joints-1))%num_joints;
   ...
   //////////////////////////////
   if (manip.haveCommunication()) attrset(COLOR_PAIR(GO_TEXT));
   else                           attrset(COLOR_PAIR(NO_GO_TEXT));
   mvprintw(n_rows-4, 0, "Comms Status");

   if (manip.haveSupply()) attrset(COLOR_PAIR(GO_TEXT));
   else                    attrset(COLOR_PAIR(NO_GO_TEXT));
   mvprintw(n_rows-4, 15, "Power Status");

   if (manip.isControllable()) attrset(COLOR_PAIR(GO_TEXT));
   else                        attrset(COLOR_PAIR(NO_GO_TEXT));
   mvprintw(n_rows-4, 30, "Control Status");

   if (manip.resetOccurred()) attrset(COLOR_PAIR(NO_GO_TEXT));
   else                       attrset(COLOR_PAIR(GO_TEXT));
   mvprintw(n_rows-4, 45, "RESET");

   if(joy.NumAxes() > 0) joy.ClearButtons();
}
...
void HDTManipulatorTestApp::PositionControlContext()
{
   ...
   /////////////////
   attrset(COLOR_PAIR(NORMAL_TEXT) | A_REVERSE); // Black text on white background for header
   mvprintw(0, 0, "Joint Position Control                                                                                  FAULT STS");fill_line(' ');
   mvprintw(1, 0, "                                                                                                        T V C C M");fill_line(' ');
   mvprintw(2, 0, "                                                                                                        E O R O I");fill_line(' ');
   mvprintw(3, 0, "      |      Position     |      Velocity     |       Torque      |    Motor Current  |  SW   |  BIT  | M L N M S");fill_line(' ');
   attron(A_UNDERLINE);
   mvprintw(4, 0, "Joint |  Target | Current |  Target | Current |  Limit  | Current |  Limit  | Current | State | Status| P T T M C");fill_line(' ');
   ...
   if (target_velocity[curr_joint_ix] < 0.0) target_velocity[curr_joint_ix] = 0.0;

   target_torque[curr_joint_ix] -= joy.Axis(ax_map[AX_TORQUE], joy_db)*0.01;
   if (target_torque[curr_joint_ix] < 0.0) target_torque[curr_joint_ix] = 0.0;

   motor_current_limit[curr_joint_ix] -= joy.Axis(ax_map[AX_MOTOR_CURRENT], joy_db)*0.01;
   if (motor_current_limit[curr_joint_ix] < 0.0) motor_current_limit[curr_joint_ix] = 0.0;
   ...
   if (joy.Button(btn_map[BTN_SEND_MOTION_CMD]))
   {
      ...
      mvprintw(13, 0, "Pressing button 5!");fill_line(' ');
      printw("%s", err.toString().c_str());fill_line(' ');
   }
   else
   {
      err = manip.setTargetPositionVelocity(target_position, zero);
      mvprintw(13, 0, "");fill_line(' ');
      printw("%s", err.toString().c_str());fill_line(' ');
   }
   ...
   ////////////////
   for(ix = params.getNumJoints()-1; ix >= 0; ix--){
        if(ix == curr_joint_ix) attrset(COLOR_PAIR(HIGHLIGHT_TEXT));
        else                    attrset(COLOR_PAIR(NORMAL_TEXT));
        printw("   % 2d : %+ 8.2f  %+ 8.2f  %+ 8.2f  %+ 8.2f  %+ 8.2f  %+ 8.2f  %+ 8.2f  %+ 8.2f ", ix,
                                                                  norm_degree_from_radian(target_position[ix]),
                                                                  norm_degree_from_radian(current_position[ix]),
                                                                  degree_from_radian(target_velocity[ix]),
                                                                  degree_from_radian(current_velocity[ix]),
                                                                  target_torque[ix],
                                                                  current_torque[ix],
                                                                  motor_current_limit[ix],
                                                                  motor_current[ix]);
        switch (sw_state[ix])
        {
           case RAD::SW_INIT:
   ...
   if (joy.PressedButton(btn_map[BTN_ACK_RESET])) manip.acknowledgeReset();
   return;
}
...
{
   HDTManipulatorTestApp app;


   if (argc > 1)
   {
       std::vector <double> jointAngles;
       jointAngles.resize(argc-1);
       std::cout << "JointAngles: " ;
       for (int i=1;i<argc;i++)
       {
           jointAngles[i-1] = atof(argv[i]);
           std::cout << jointAngles[i-1] << " ";
       }
       app.Initialize();
       app.SetInitialPositionDegrees(jointAngles);
       app.Run();
