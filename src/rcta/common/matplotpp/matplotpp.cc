...
float MatPlot::cty2(double y) {
    ...
    if (ca->YScale == 1) {	//log
        return ca->Position[1]
            + ca->Position[3] * (log10(y) - log10(ca->YLim[0])) / (log10(ca->YLim[1]) - log10(ca->YLim[0]));
    }
}
// axes coordination
...
void MatPlot::print() {
    ...
    while (state == GL2PS_OVERFLOW) {
        ...
        GL2PS_USE_CURRENT_VIEWPORT, GL_RGBA, 0, NULL, 0, 0, 0, buffsize, fp, "out.eps");
        display();
        state = gl2psEndPage();
    }
    fclose(fp);
...
void MatPlot::display_axes() {
    if (is_debug1) {
        printf("mode: %d handle: %4d Axes  #Children %zd\n", mode, ca->id, ca->Children.size());
    }
...
void MatPlot::display_axes_2d() {
    ...
    if (ca->Box) {
        ...
        if (ca->TickLabel) {
            for (int i = 0; i < ca->YTick.size(); ++i) {
                ...
                ptext(l - (float) num_char * char_w / window_w - offset, cty2(ca->YTick[i]) - 0.5 * char_h / window_h,
                    ctmp);
            }
        }
    }	//Box
    ...
    ptext(l + w / 2.0 - (float) num_char * char_w / window_w / 2.0, b + h + offset, ca->Title);

    //XLabel
    num_char = ca->XLabel.length();
    ptext(l + w / 2.0 - (float) num_char * char_w / window_w / 2.0, b - offset - 2.0 * char_h / window_h, ca->XLabel);
    ...
    glViewport((int) (ca->Position[0] * window_w), (int) (ca->Position[1] * window_h),
...
void MatPlot::display_axes_3d() {
    ...
    glViewport((int) (ca->Viewport3d[0] * window_w), (int) (ca->Viewport3d[1] * window_h),
        (int) (ca->Viewport3d[2] * window_w), (int) (ca->Viewport3d[3] * window_h));

    glLoadIdentity();
    //glOrtho(-1.7, 1.7, -1.7, 1.7, -1.5, 3);
    glOrtho(-1.8, 1.8, -1.8, 1.8, -1.5, 3);

    gluLookAt(cos(ca->cta * PI / 180) * cos(ca->phi * PI / 180), sin(ca->cta * PI / 180) * cos(ca->phi * PI / 180),
        sin(ca->phi * PI / 180),
        //gluLookAt(CameraPosition[0],CameraPosition[1],CameraPosition[2],
        ca->CameraTarget[0], ca->CameraTarget[1], ca->CameraTarget[2], ca->CameraUpVector[0], ca->CameraUpVector[1],
        ca->CameraUpVector[2]);
...
void MatPlot::display_line() {
    ...
    if (ca->View == 1) {
        glBegin (GL_LINE_STRIP);
        for (int i = 0; i < cl->XData.size(); ++i) {
            glVertex3d(ct3x(cl->XData[i]), ct3y(cl->YData[i]), ct3z(cl->ZData[i]));
        }
        glEnd();
    }
}
...
void MatPlot::display_surface() {
    if (is_debug1) {
        printf("mode: %d handle: %4d Surface type %d, Z.size %zd\n", mode, cs->id, cs->type, cs->ZData.size());
    }
        ...
        if (cs->FaceColor != "none") {
            for (int i = 0; i < nyj - 1; ++i) {
                for (int j = 0; j < nxj - 1; ++j) {
                    ...
                    glVertex2d(ctx(cs->XData[0][j + 1]), cty(cs->YData[0][i + 1]));
        ...
        if (cs->FaceColor != "none") {
            for (int i = 0; i < nxi - 1; ++i) {
                for (int j = 0; j < nxj - 1; ++j) {
                    ...
                    glBegin (GL_QUADS);
                    glVertex2d(ctx(cs->XData[i][j]), cty(cs->YData[i][j]));
                    glVertex2d(ctx(cs->XData[i][j + 1]), cty(cs->YData[i][j + 1]));
                    glVertex2d(ctx(cs->XData[i + 1][j + 1]), cty(cs->YData[i + 1][j + 1]));
                    glVertex2d(ctx(cs->XData[i + 1][j]), cty(cs->YData[i + 1][j]));
                    glEnd();
                }
            }
        }
        ...
        if (cs->FaceColor != "none") {
            for (int i = 0; i < ny - 1; ++i) {
                for (int j = 0; j < nx - 1; ++j) {
                    ...
                    glVertex3d(ct3x(cs->XData[0][j]), ct3y(cs->YData[0][i]), ct3z(cs->ZData[i][j]));
                    glVertex3d(ct3x(cs->XData[0][j]), ct3y(cs->YData[0][i + 1]), ct3z(cs->ZData[i + 1][j]));
                    glVertex3d(ct3x(cs->XData[0][j + 1]), ct3y(cs->YData[0][i + 1]), ct3z(cs->ZData[i + 1][j + 1]));
                    glEnd();
                    glBegin(GL_TRIANGLES);
                    glVertex3d(ct3x(cs->XData[0][j]), ct3y(cs->YData[0][i]), ct3z(cs->ZData[i][j]));
                    glVertex3d(ct3x(cs->XData[0][j + 1]), ct3y(cs->YData[0][i]), ct3z(cs->ZData[i][j + 1]));
                    glVertex3d(ct3x(cs->XData[0][j + 1]), ct3y(cs->YData[0][i + 1]), ct3z(cs->ZData[i + 1][j + 1]));
                    glEnd();
                }
            }
        ...
        if (cs->FaceColor != "none") {
            for (int i = 0; i < ny - 1; ++i) {
                for (int j = 0; j < nx - 1; ++j) {
                    ...
                    glBegin (GL_TRIANGLES);
                    glVertex3d(ct3x(cs->XData[i][j]), ct3y(cs->YData[i][j]), ct3z(cs->ZData[i][j]));
                    glVertex3d(ct3x(cs->XData[i][j + 1]), ct3y(cs->YData[i][j + 1]), ct3z(cs->ZData[i][j + 1]));
                    glVertex3d(ct3x(cs->XData[i + 1][j + 1]), ct3y(cs->YData[i + 1][j + 1]),
                        ct3z(cs->ZData[i + 1][j + 1]));
                    glEnd();

                    glBegin(GL_TRIANGLES);
                    glVertex3d(ct3x(cs->XData[i][j]), ct3y(cs->YData[i][j]), ct3z(cs->ZData[i][j]));
                    glVertex3d(ct3x(cs->XData[i + 1][j]), ct3y(cs->YData[i + 1][j]), ct3z(cs->ZData[i + 1][j]));
                    glVertex3d(ct3x(cs->XData[i + 1][j + 1]), ct3y(cs->YData[i + 1][j + 1]),
                        ct3z(cs->ZData[i + 1][j + 1]));
                    glEnd();
                }
...
void MatPlot::display_patch() {
    if (is_debug1) {
        printf("display:%4d Patch :type %d, Faces.size %zd\n", cp->id, cp->type, cp->Faces.size());
    }
...
void MatPlot::display_patch_3d() {
    ...
    for (int i = 0; i < nf; ++i) {
        ...
        if (cp->EdgeColor != "none") {
            ...
            glBegin (GL_LINE_LOOP);
            for (int iv = 0; iv < nv; ++iv) {
                glVertex3d(ct3x(cp->XData[i][iv]), ct3y(cp->YData[i][iv]), ct3z(cp->ZData[i][iv]));
            }
            glEnd();
