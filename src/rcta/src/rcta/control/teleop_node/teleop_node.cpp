...
TeleopNode::RunResult TeleopNode::run()
{
    ...
    while (ros::ok() && !shutdown_) {
        ...
        rcta::TeleopDiagnosticStatus status;
        ...
        status.at_limit = rcta::TeleopDiagnosticStatus::_at_limit_type(joint_names_.size(), false);
...
bool TeleopNode::init()
{
    ...
    diagnostic_pub_ = nh_.advertise<rcta::TeleopDiagnosticStatus>("teleop_diagnostics", 5);
    ack_reset_pub_ = nh_.advertise<rcta::AcknowledgeReset>("acknowledge_hdt_reset", 1);
...
{
    rcta::AcknowledgeReset reset_msg;
