...
bool HDTManipulatorTestApp::GetJointState(std::vector <double> & positions,
...
void HDTManipulatorTestApp::Update()
{
    ...
    for (size_t i = 0; i<current_position.size() ; i++)
    {
        ...
        else
        {
            ...
            current_position[i] = (current_position[i]-position_offset[i])*alpha + current_position_raw[i]*(1-alpha) + position_offset[i];
        }
    }
