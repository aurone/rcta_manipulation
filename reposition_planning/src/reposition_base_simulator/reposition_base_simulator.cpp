...
void RepositionBaseSimulator::goal_callback()
{
    ...
    ROS_INFO("Object Yaw: %0.3f degs", smpl::angles::to_degrees(object_yaw));

    // we want the heading of the object to be offset 60 degrees from the heading of the base
    const double canonical_heading_diff_rad = smpl::angles::to_radians(60.0);
    ...
    auto compare_poses = [&] (const geometry_msgs::PoseStamped& a, const geometry_msgs::PoseStamped& b)
    {
        ...
        double canonical_heading_diff_a = fabs(canonical_heading_diff_rad - smpl::angles::shortest_angle_diff(object_yaw, a_yaw));
        double canonical_heading_diff_b = fabs(canonical_heading_diff_rad - smpl::angles::shortest_angle_diff(object_yaw, b_yaw));
    ...
    for (const geometry_msgs::PoseStamped& pose : candidate_base_poses) {
        ...
        ROS_INFO(" -> Yaw: %0.3f", smpl::angles::to_degrees(y));
    }
