    ...
    watchdog_()
{
    ...
    std::string path;
    const std::string package_name = "hdt_control_rviz_plugins";
...
void ControllerStatusPanelWidget::joint_states_callback(const sensor_msgs::JointState::ConstPtr& msg)
{
    ...
    for (std::size_t i = 0; i < msg->name.size(); ++i) {
        ...
        past_joint_states_[jidx].push_back(smpl::angles::to_degrees(joint_val));
    }
...
void ControllerStatusPanelWidget::raw_joint_states_callback(const sensor_msgs::JointState::ConstPtr& msg)
{
    ...
    for (std::size_t i = 0; i < msg->name.size(); ++i) {
        ...
        past_raw_joint_states_[jidx].push_back(smpl::angles::to_degrees(joint_val));
    }
...
void hdt::ControllerStatusPanelWidget::initialize_gui()
{
    ...
    ui->raw_joint_states_plot->setAxisScale(QwtPlot::yLeft, -smpl::angles::to_degrees(M_PI), smpl::angles::to_degrees(M_PI));
    ...
    ui->joint_states_plot->setAxisScale(QwtPlot::yLeft, -smpl::angles::to_degrees(M_PI), smpl::angles::to_degrees(M_PI));
}
